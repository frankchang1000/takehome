...
def compose_messages(repo_url: str, directives: List[str], as_json: bool) -> List[dict]:
    system_text = (
        "You are a senior SDK analyst. Provide accurate, concise overviews with inline citations when available."
    )
    user_text = (
        f"Repository: {repo_url}\n\n"
        "Goals:\n- Identify entry clients and service namespaces\n- Core resources/entities and key methods\n- Authentication and required configuration\n- Usage patterns and examples\n- Pagination, rate limits, error handling\n- Versioning/compatibility notes\n- Include links/citations in-line if using web search\n\n"
        "Directives:\n- " + "\n- ".join(directives)
    )
    if as_json:
        user_text += "\n\nOutput format: JSON with keys repo_url, package_name, entry_clients, services, auth, core_resources, usage_patterns, pagination, errors, versioning, links, model_notes."
    return [
...
def call_openai_multi_stage_analysis(*, model: str, repo_url: str, allowed_domains: List[str], verbose: bool, retries: int = 2) -> Tuple[str, List[str]]:
    """Multi-stage analysis to avoid token limits"""
    import time
    ...
    client = OpenAI()
    if verbose:
        print(f"🔌 Initialized OpenAI client for multi-stage analysis")
        print(f"🎯 Target model: {model}")
        print(f"🌐 Allowed domains: {allowed_domains}")
    ...
    # Stage 1: Basic package info and structure
    if verbose:
        print("📋 Stage 1: Basic package info and structure")
    
    stage1_prompt = f"""Analyze this SDK repository for basic information: {repo_url}

Focus on:
- Package name and installation method
- Main entry point class/module
- Basic repository structure and purpose

Search for official documentation, README, and setup instructions.

Respond with JSON containing:
{{
  "package_name": "exact PyPI package name",
  "main_entry_point": "main class or module name", 
  "installation": "pip install command",
  "description": "what this SDK does",
  "repo_url": "{repo_url}"
}}"""
    ...
    all_sources.extend(stage1_sources)
    analysis_parts["basic_info"] = stage1_result
    
    # Stage 2: Authentication methods
    if verbose:
        print("🔐 Stage 2: Authentication methods")
        
    stage2_prompt = f"""Analyze authentication methods for this SDK: {repo_url}

Focus on:
- All available authentication methods (tokens, API keys, OAuth, etc.)
- Code examples for each method
- Authentication best practices

Search for authentication documentation and security guides.
  ...
  "authentication": {{
    "methods": [
      {{
        ...
        "code_example": "complete Python code with imports",
    ...
    all_sources.extend(stage2_sources)
    analysis_parts["authentication"] = stage2_result
    
    # Stage 3: Main classes and core operations
    if verbose:
        print("⚙️  Stage 3: Main classes and operations")
        
    stage3_prompt = f"""Analyze the main classes and operations for this SDK: {repo_url}

Focus on:
- Primary classes and their purposes
- Key methods for each class  
- CRUD operations and resource management
- Method parameters and return types

Search for API reference and class documentation.
...
{{
  ...
  "resource_operations": [
    ...
    all_sources.extend(stage3_sources)
    analysis_parts["classes_operations"] = stage3_result
    
    # Stage 4: Usage patterns and examples
    if verbose:
        print("💡 Stage 4: Usage patterns and examples")
        
    stage4_prompt = f"""Analyze common usage patterns and examples for this SDK: {repo_url}
...
Search for tutorials, guides, and example code.
...
{{
  "usage_patterns": [
    {{
      ...
      "code_example": "complete working example",
  ...
  "best_practices": ["practice1", "practice2"]
    ...
    all_sources.extend(stage4_sources)
    analysis_parts["usage_patterns"] = stage4_result
    
    # Combine all results
    if verbose:
        print("🔧 Combining analysis results...")
        
    combined_analysis = combine_analysis_stages(analysis_parts, repo_url)
    
    return combined_analysis, all_sources
...
def combine_analysis_stages(parts: dict, repo_url: str) -> str:
    """Combine multiple analysis stages into final JSON"""
    ...
    try:
        ...
        for key, content in parts.items():
            try:
                ...
                elif key == "classes_operations":
                    classes_operations = parsed
                elif key == "usage_patterns":
                    usage_patterns = parsed
        ...
        combined = {
            "repo_url": repo_url,
            "package_name": basic_info.get("package_name", "unknown"),
            "main_entry_point": basic_info.get("main_entry_point", ""),
            "installation": basic_info.get("installation", ""),
            "description": basic_info.get("description", ""),
            "authentication": authentication.get("authentication", {"methods": []}),
            "main_classes": classes_operations.get("main_classes", []),
            "resource_operations": classes_operations.get("resource_operations", []),
            "usage_patterns": usage_patterns.get("usage_patterns", []),
            "error_handling": usage_patterns.get("error_handling", ""),
            "best_practices": usage_patterns.get("best_practices", [])
        }
        
        return json_module.dumps(combined, indent=2)
        ...
        return json_module.dumps({
            ...
            "error": f"Failed to combine analysis stages: {str(e)}",
            "raw_parts": parts
        }, indent=2)
...
def compose_input_text(repo_url: str, directives: List[str], as_json: bool) -> str:
    if as_json:
        return f"""Repository: {repo_url}

Task: Use web search to thoroughly read the repository README, official documentation, examples, and API reference. Extract comprehensive detailed information needed to build MCP server tools for this SDK.

CRITICAL: Focus on documentation that shows:
- How to authenticate/initialize the SDK
- Main classes and their methods with parameters
- CRUD operations (Create, Read, Update, Delete) for different resources
- Real code examples showing actual usage
...
{{
  ...
  "main_entry_point": "string (main class like 'Github')",
  "installation": "string (pip install command)",
  "authentication": {{
    "methods": [
      {{
        "name": "string (e.g., 'Token Authentication')",
        "description": "string (detailed description)",
        "code_example": "string (complete Python code with imports)",
  ...
  "main_classes": [
    {{
      "name": "string (exact class name)",
      "description": "string (what it represents)",
      "methods": [
        {{
          "name": "string (exact method name)",
          "description": "string (what it does)",
          "parameters": ["string (param: type descriptions)"],
          "returns": "string (return type and description)",
          "example": "string (complete code example)"
        }}
      ]
    }}
  ],
  "resource_operations": [
    {{
      "resource_type": "string (e.g., 'User', 'Project', 'Item', 'Document')",
      "operations": [
        {{
          "action": "string (e.g., 'create', 'get', 'list', 'update', 'delete')",
          "method_name": "string (exact method name)",
          "description": "string (what it does)",
          "parameters": [
            {{
          ...
          "returns": "string (return type and structure)",
  ...
  "common_patterns": [
    {{
      "pattern_name": "string (e.g., 'List Items', 'Search Resources', 'Bulk Operations')",
      "description": "string (what this pattern accomplishes)",
      "code_example": "string (complete working code)",
      "variations": ["string (alternative approaches)"]
    }}
  ],
  "pagination": {{
    "method": "string (how pagination works)",
    "example": "string (code example of pagination)"
  }},
  "rate_limits": {{
    "how_to_check": "string (method to check rate limits)",
    "example": "string (code example)",
    "limits": "string (typical limits info)"
  }},
  "error_handling": {{
    "exception_types": ["string (exception class names)"],
    "example": "string (code example of error handling)"
  }},
  "advanced_features": [
    {{
...
IMPORTANT: Extract REAL method names, parameter lists, and complete working code examples from the documentation. Look for:
- Getting started guides and tutorials
- API reference documentation  
- Examples or cookbook sections
- Method documentation with signatures
- Error handling examples
- Pagination and rate limiting information

Return valid JSON only."""
    else:
        base = (
            f"Repository: {repo_url}\n\n"
            "Summarize the SDK with sections: Entry clients; Services; Core resources & key methods; Auth; Usage patterns; Pagination & errors; Versioning; Links.\n\n"
            "Directives:\n- " + "\n- ".join(directives)
        )
        return base
...
def main() -> None:
    ...
    # Compose input text for Responses API (always markdown)
    print("📄 Composing analysis prompt...")
    input_text = compose_input_text(repo_url, directives, False)  # Always use markdown format
